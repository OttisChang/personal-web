...
knowledge_base_agent = Agent(
    ...
    label="知識庫 Agent",
    description="查詢內部知識庫文件（目前收錄：《Pro Git》、《Google Cloud: 生成式 AI 瞭解基礎概念》），回答文件內容相關問題",
    kind="tool_agent",
    tools=("search_documents",),
    instruction=(
        f"{_ZH_TW} 你是知識庫助手，請根據以下資料摘錄內容回答使用者的問題，"
        "回答內文不需要逐句標註頁碼。回答的最後請另起一段，針對「資料」最下方列出的每一份文件，"
        "各自輸出一組來源說明，格式須為：\n"
        "參考資料來源：《書名》，頁碼範圍 p.xx-xx\n"
        "文本連結：<連結>\n"
        "書名、頁碼範圍與 <連結> 請一字不漏照抄資料最下方「（《書名》，頁碼範圍...，文本連結：...）」"
        "這幾行提供的內容，不可自行編造或修改；若資料中列出兩份以上的文件來源，請分別各自成一組。"
        "若摘錄內容不足以回答，誠實告知使用者，不要編造資料中沒有的內容，此時不需加上這些來源行。"
    ),
    judging_hints=(
        "使用者問 git 指令、概念、用法（branch、merge、rebase、commit、submodule、reflog 等） → knowledge_base_agent",
        "使用者問生成式 AI 基礎概念相關問題 → knowledge_base_agent",
    ),
    routing_examples=('{"agent": "knowledge_base_agent"}',),
)
...
root_agent = RootAgent(
    name="root",
    sub_agents=(weather_agent, financial_agent, knowledge_base_agent, travel_brainstormer, attractions_planner, web_search_agent),
)
